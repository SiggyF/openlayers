                ...
                (function callback() {
                    ...
                    OpenLayers.Request.POST({
                        ...
                        success: function(response) {
                            // Try if we can parse the result as an WPSExecuteResponse Document
                            var parsed = new OpenLayers.Format.WPSExecuteResponse().read(response.responseText);
                            var result;
                            // If we can't parse the response as an executeresponse, assume it's the direct output
                            if (parsed.error) {
                            ...
                            if (options.success) {
                                ...
                                outputs.responseText = response.responseText;
        ...
        if (data instanceof OpenLayers.WPSProcess.ChainLink) {
            ...
            if (complexData) {
                ...
                try {
                    ...
                    // in case of TypeError, just store data (binary)
                    value = data;
                }
